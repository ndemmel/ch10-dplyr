...
pulitzer_circulation <- read.csv(file = "data/pulitzer-circulation-data.csv", stringsAsFactors = FALSE)
...
# when newspapers had a -100% change in circulation, does that mean they went out of business?
...
mutate(pulitzer_circulation, Pulitzer.Prize.Change = Pulitzer.Prize.Winners.and.Finalists..2004.2014 - Pulitzer.Prize.Winners.and.Finalists..1990.2003)

# What was the name of the publication that has the most winners between 
# 2004-2014?
filter(pulitzer_circulation, max(Pulitzer.Prize.Winners.and.Finalists..2004.2014) == Pulitzer.Prize.Winners.and.Finalists..2004.2014) %>% select(Newspaper)
...
# dplyr to figure out the answer!
  ...
  # is there a correlation between the change in the number of Pulitzer prizes
  # won and the change in circulation of a newspaper?
